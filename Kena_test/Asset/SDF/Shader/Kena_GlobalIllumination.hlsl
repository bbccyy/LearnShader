...
half3 SubtractDirectMainLightFromLightmap(Light mainLight, half3 normalWS, half3 bakedGI)
{
    ...
    //a）保留其他烘焙灯光和灯光反弹
    //b）消除几何体上远离光线的阴影
    //2）夹紧用户定义的ShadowColor。
    //3）拾取原始光照贴图值（如果它是最暗的值）。

    //1）提供良好的照明估计，就像在烘焙过程中光线会被遮挡一样。
    //我们只减去主方向灯。这在下面的缴款期限中进行了说明。
    // 1) Gives good estimate of illumination as if light would've been shadowed during the bake.
    // We only subtract the main direction light. This is accounted in the contribution term below.
    half shadowStrength = GetMainLightShadowStrength();
    half contributionTerm = saturate(dot(mainLight.direction, normalWS));
    half3 lambert = mainLight.color * contributionTerm;                 // 在这里改了NdotL 更亮点
...
half3 normalWS, half3 viewDirectionWS)
{
    ...
    half3 R12 = indirectDiffuse.xyz * 1.111111;  //R10可以认为是采样的Diffuse颜色          Kena
    half3 NoV_soft = 0.85 * (NoV - 1) * (1 - (R12 - 0.78 * (R12 * R12 - R12))) + 1;     //类似col - t*(col^2 - col) -> 这种模式的颜色操作等效于对原始颜色进行 "非线性提亮"
