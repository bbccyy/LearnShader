...
namespace Unity.MergeInstancingSystem.SpaceManager
{
    public static class BoundsExtension
    {
        ...
        /// 判断两个包围盒所属关系
        /// </summary>
        /// <param name="bounds">小包围盒</param>
        /// <param name="target">大包围盒</param>
        /// <returns></returns>
        public static bool IsPartOf(this Bounds bounds, Bounds target)
        {
            ...
            bool isIBox = dis_x < target.size.x && dis_y < target.size.x && dis_z < target.size.x;
            // if (!isInBOX && isLess)
            // {
            //     var dis = Vector3.Distance(bounds.center, target.center);
            //     return dis < x_length * 05f;
            // }
            return isIBox;
        }
    }
