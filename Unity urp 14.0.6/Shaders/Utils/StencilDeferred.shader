...
//#if 1
    ...
    FRAMEBUFFER_INPUT_FLOAT(GBUFFER3); //对应DeferredLights.DeferredInputAttachments[3] -> DepthAttachment 
#else
    ...
    //TODO: 新加入2个Kena uber pass，放在已有pass的后面，这样不会影响已有pass的序号，
    //单独在C#端DefferredLights内加入指令渲染kenaPass，要求参考Lit Pass，需要带上光源信息，变换矩阵，探针信息等数据 
    //参考如下DeferredShading方法，结合kenaPass原有逻辑，改写出能兼容Unity Deferred shading的PS 
    half4 DeferredShading(Varyings input) : SV_Target
    {
        ...
        float d        = LOAD_FRAMEBUFFER_INPUT(GBUFFER3, input.positionCS.xy).x; //证明 GBUFFER3 确实是Depth 
